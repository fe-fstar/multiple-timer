...
export async function login(_, formData) {
  ...
  let username_xor_email =
    validatedFields.data.type === "email" ? "email" : "username";
  ...
  let userFound = await db
    ...
    .where(
      eq(
        users[username_xor_email],
        validatedFields.data.username_xor_email.value
      )
    )
    .limit(1);
...
export async function register(_, formData) {
  ...
  try {
    ...
    if (!validatedFields.success) {
      for (let errorList of Object.values(
...
export async function updatePassword(_, formData) {
  ...
  try {
    ...
    if (formDataObject.password !== formDataObject.confirm_password) {
      errors.push("passwords_mismatch");
    }

    // Password validation (e.g., minimum length check, pattern check)
    if (formDataObject.password.length < 8) {
      errors.push("password_minmax");
    }
    ...
    // Update the password in the database (assuming you send userId in formData)
    await db
      .update(users)
      .set({ password: hashedPassword })
      .where(eq(users.id, user.id)); // Assuming you send userId as part of the form data
